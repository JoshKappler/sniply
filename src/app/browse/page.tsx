...
function BrowseContent() {
  ...
  const [bannerDismissed, setBannerDismissed] = useState(() => {
    try { return localStorage.getItem("sniply_profile_banner_dismissed") === "1"; } catch { return false; }
  });
      ...
      <main className="max-w-[1200px] mx-auto px-6 py-8 animate-fade-in-up">
        ...
        {!loading && userRole === "customer" && !customerProfile && !bannerDismissed && (
          <div className="mb-6 flex items-center gap-3 bg-[var(--color-primary)]/6 border border-[var(--color-primary)]/20 rounded-xl px-5 py-3.5">
            ...
            <button onClick={() => { setBannerDismissed(true); try { localStorage.setItem("sniply_profile_banner_dismissed", "1"); } catch {} }} className="text-[var(--color-primary)]/50 hover:text-[var(--color-primary)] transition-colors shrink-0" aria-label="Dismiss">
        ...
        {!loading && userRole === "customer" && customerProfile && (customerProfile.stylePrefs?.length ?? 0) === 0 && !bannerDismissed && (
          <div className="mb-6 flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-5 py-3.5">
            ...
            <button onClick={() => { setBannerDismissed(true); try { localStorage.setItem("sniply_profile_banner_dismissed", "1"); } catch {} }} className="text-amber-400 hover:text-amber-600 transition-colors shrink-0" aria-label="Dismiss">
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
              </svg>
            </button>
